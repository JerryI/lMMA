...
node* var(node* itself, vector<node*> &args, unsigned id, vector<node*> &library) {
    ...
    for (int i=0; i<library.size(); ++i) {
        if (library[i]->data.id == id) {
            ...
            return evaluate(library[i], library); 
        }
    }
    ...
    for (int i=0; i<library.size(); ++i) 
        ...
        if (library[i]->data.id == id) {
            ...
            for (int j=0; j<args.size(); ++j) {
                ...
                buffer->data.args.push_back(evaluate(args[j], library));
            ...
            //cout <<  " befo fun" << '\n' << watch << '\n';
            node* func_run = evaluate(library[i], local);
            //cout <<  " after fun" << '\n' << watch << '\n';
            //cnt++;
            return func_run;
        }
...
node* evaluate(node *data, vector<node*> &library) {
    ...
    switch(data->type) {
        ...
        case CONT: {
            return evaluate(data->data.args[0], library); 
        }   
        //functions
        default:
