...
typedef struct node{
    ...
    //when data is already lost its original representation
    bool overwritten = false;

    bool readonly = false;

    friend inline void allocate(node* t);
    friend inline void generate_fingerprint(node* t);

    //default constructor
    node(TYPE t) {
        this->type = t;

        //call constructor hash()
    ...
    node* shallowclone() {
        //clone only when isnt overwritten
        if (this->overwritten) return this;
        node* ptr = this->clone();
        ptr->overwritten = true;
        return ptr;
    }
...
inline void generate_fingerprint(node* t) {
    t->fingerprint = fingerprint_strategies[t->type](t);
}
